...
class source:
		...
		try:
			...
			for title in titles:
				try:
					...
					oRequest = requests.get(query)
					sHtmlContent = oRequest.text
			...
			if season > 0:
				...
				url = re.findall(r'(.*?)s\d', url)[0] + 's%se%s' % (s, e)
			...
			for hoster, link in r:
				isBlocked, sDomain, url, prioHoster = isBlockedHoster(link)
				if isBlocked: continue
				## sources.append({'source': hoster, 'quality': quality, 'language': 'de', 'url': sUrl, 'direct': True, 'prioHoster': prioHoster})
				sources.append({'source': hoster, 'quality': quality, 'language': 'de', 'url': url, 'info': '', 'direct': True, 'priority': int(self.priority), 'prioHoster': prioHoster})
	...
	def chk_year(self, i, year):
		try:
			...
			oRequest = cRequestHandler(query)
			sHtmlContent = oRequest.request()
			found_year = re.search(r'licht:.*?(\d{4})', sHtmlContent).group(1)
