...
out = repo / 'zips/test/plugin.video.xship-2026.09.14.93-SCRAPERCORETEST.zip'
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    text, count = re.subn(r'(<addon\s+id="plugin\.video\.xship"\s+version=")[^"]+', r'\g<1>2026.09.14.93', text, count=1)
    if count != 1:
    ...
    # Guardrails: ensure the intended fixes are really present in the packaged code.
    movie2k_text = (scrapers/'movie2k.py').read_text(encoding='utf-8')
    compat_text = (lib/'hoster_compat.py').read_text(encoding='utf-8')
    if "Season" not in movie2k_text or "Mehrsprachig" not in movie2k_text:
        raise RuntimeError('Movie2k language fix missing')
    if '[0-9A-Za-z_-]+' not in compat_text:
        raise RuntimeError('KinoGer id fix missing')
