...
def _decode_link_value(value):
    """Decode MeineCloud mirror values when the site stores them as Base64."""
    value = unescape(str(value or '')).strip().replace('\\/', '/')
    if not value:
        return ''

    # Series pages currently expose normal URLs while movie pages expose
    # Base64 strings such as Ly9zdXBlcnZpZGVvLmNjL2Uv... (= //supervideo.cc/e/...).
    if value.startswith(('http://', 'https://', '//', '/')):
        return value

    compact = re.sub(r'\s+', '', value)
    if len(compact) >= 12 and re.fullmatch(r'[A-Za-z0-9_+/=-]+', compact):
        try:
            padded = compact + ('=' * (-len(compact) % 4))
            decoded = base64.b64decode(padded, altchars=b'-_', validate=False).decode('utf-8', 'strict').strip()
...
def _normalize_link(value, base_link):
    value = _decode_link_value(value)
    if not value:
        return ''
    if value.startswith('//'):
        return 'https:' + value
...
def get_movie_links(imdb, base_link=DEFAULT_BASE):
    ...
    # Cache only successful lookups. A transient challenge/empty response
    # must not poison later retries for the whole Kodi process.
    if links:
        with _cache_lock:
