    ...
    def run(self, titles, year, season=0, episode=0, imdb='', hostDict=None):
        ...
        try:
            ...
            for i in titles:
                if not i:
                    continue
                if i not in search_titles:
                    search_titles.append(i)
                for _sep in (' – ', ' — ', ' - ', ': '):
                    if _sep in i:
                        _short = i.split(_sep, 1)[0].strip()
                        if _short and _short not in search_titles:
                            search_titles.append(_short)
