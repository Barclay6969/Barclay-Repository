...
class source:
    ...
    def _title_match(candidate, clean_titles):
        ...
        for wanted in clean_titles:
            if not wanted:
                continue
            if min(len(cleaned), len(wanted)) >= 5 and SequenceMatcher(None, cleaned, wanted).ratio() >= 0.92:
                return True
        return False

    def _candidate_matches(self, movie, clean_titles, year, season_num):
        title = str(movie.get('title') or '').strip()
        mid = str(movie.get('_id') or '').strip()
        ...
        if season_num:
            ...
            return self._title_match(base, clean_titles)
