...
class source:
    ...
    def _playlist_from_embed(self, embed_url, referer, stream_language):
        ...
        def _playlist_candidate(use_b, use_origin, label):
            query_prefix = 'b=1&' if use_b else ''
            candidate_url = '%s?%stoken=%s&expires=%s&h=1&lang=%s' % (
                playlist_base,
                query_prefix,
                token_match.group(1),
        ...
        # Browser HAR (FROM S01E01): when the embed exposes a literal playlist
        # URL, TV succeeds without b=1 and without Origin.
        # Reacher differs: its embed has no literal playlist URL, so the playlist
        # must be derived from the embed ID. For that fallback path, first keep the
        # established b=1/Origin request shape, then try the minimal variant.
        if is_tv and not derived_playlist:
            candidates = [
                _playlist_candidate(False, False, 'TV-HAR literal: ohne b=1 / ohne Origin'),
            ]
        elif is_tv and derived_playlist:
            candidates = [
            ...
            candidates = [
                _playlist_candidate(True, True, 'Movie/default: b=1 / Origin'),
            ]
